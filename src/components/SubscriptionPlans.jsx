...
import { CheckCircle2, Gift, Clock, CreditCard, CalendarDays, MapPin, Info, ShoppingBag } from 'lucide-react';
import { Link } from 'react-router-dom'; // Toast me checkout link ke liye

// 🔴 Props mein cart aur setCart receive kar rahe hain
const SubscriptionPlans = ({ cart = [], setCart }) => {
  const [activeTab, setActiveTab] = useState('full');
  
  // 🔴 2030 Look wale Custom Alert (Toast) ke liye state
  const [toast, setToast] = useState({ show: false, planName: '' });
  ...
  // 🔴 Handle Subscription (Add to Cart logic)
  const handleSubscribe = (plan) => {
    ...
    // Number se comma hatana ('5,280' -> 5280)
    const numericPrice = parseInt(plan.newPrice.replace(/,/g, ''), 10);
    const passType = activeTab === 'full' ? 'Full Day Pass' : 'Daily Duo Pass';
    const itemName = `${passType}: ${plan.name}`;
    const itemId = `sub_${activeTab}_${plan.name.replace(/\s+/g, '')}`;
    ...
    if (existingItem) {
      ...
      setCart([...cart, { 
        ...
        price: numericPrice, 
        qty: 1, 
        desc: `Ratio: ${plan.ratio} | Free: ${plan.free}` 
      }]);
    }

    // 🔴 Show 2030 Alert (Toast)
    setToast({ show: true, planName: itemName });
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 mb-16">
        {activePlans.map((plan, index) => {
          ...
          return (
            <div 
              key={index} 
              className={`relative ${theme.bg} rounded-xl p-8 border-2 transition-all duration-300 hover:-translate-y-2 hover:shadow-xl backdrop-blur-sm flex flex-col h-full ${plan.popular ? `${theme.border} shadow-[0_8px_30px_rgba(0,0,0,0.12)] scale-105 z-10` : 'border-white/50 shadow-sm'}`}
            >
              {plan.popular && (
              ...
              <div className={`${theme.box} rounded-xl p-4 mb-8 border flex-grow`}>
                <div className="flex items-start gap-3">
                  <Gift size={18} className={`${theme.icon} mt-0.5 shrink-0`} />
                  <div>
              ...
              <button 
                ...
                className={`w-full py-3.5 rounded-xl font-bold transition-all text-white shadow-md hover:shadow-lg active:scale-95 flex items-center justify-center gap-2 ${theme.btn}`}
              >
        ...
        <div className="fixed bottom-6 right-6 md:bottom-10 md:right-10 z-[999999] animate-in slide-in-from-bottom-8 fade-in duration-300">
          <div className="bg-[#0f2405]/80 backdrop-blur-xl border border-[#a4e363]/40 p-4 rounded-2xl shadow-[0_0_30px_rgba(164,227,99,0.3)] flex items-center gap-4">
            ...
            <div className="flex flex-col pr-4">
              ...
              <span className="text-white font-semibold">{toast.planName} added!</span>
            </div>
