...
const [activeDay, setActiveDay] = useState(0);
  ...
  const menuData = [
    ...
    },
    {
      ...
      comboVeg: 'Matar Paneer'
    }
  ];

  useEffect(() => {
    const todayIndexJS = new Date().getDay(); 
    const adjustedIndex = todayIndexJS === 0 ? 6 : todayIndexJS - 1; 
    setActiveDay(adjustedIndex);
  }, []);
            ...
            <div className="animate-in fade-in zoom-in-95 duration-300">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-10">
                ...
                <div className="bg-white/60 p-5 rounded-xl border border-gray-100 shadow-sm hover:shadow-md transition-shadow">
                  ...
                  <p className="text-gray-800 font-medium leading-relaxed pr-4 mb-4 min-h-[50px]">
                    {menuData[activeDay].breakfast}
                  </p>
                  <button 
                    onClick={() => handleAddToCart('Breakfast', menuData[activeDay].breakfast, 60)}
                    className="flex items-center gap-1.5 text-sm font-bold bg-[#7CB342]/10 text-[#7CB342] hover:bg-[#7CB342] hover:text-white px-4 py-2 rounded-lg transition-colors border border-[#7CB342]/30 w-fit"
                  >
                ...
                <div className="bg-white/60 p-5 rounded-xl border border-gray-100 shadow-sm hover:shadow-md transition-shadow">
                  ...
                  <button 
                    onClick={() => handleAddToCart('Lunch', menuData[activeDay].lunch, 80)}
                    className="flex items-center gap-1.5 text-sm font-bold bg-[#7CB342]/10 text-[#7CB342] hover:bg-[#7CB342] hover:text-white px-4 py-2 rounded-lg transition-colors border border-[#7CB342]/30 w-fit"
                  >
                    <Plus size={16} strokeWidth={2.5} /> Add to Order
                  </button>
                ...
                <div className="relative bg-gradient-to-br from-green-50 to-white p-5 rounded-xl border-2 border-[#7CB342]/40 shadow-sm hover:shadow-md transition-shadow">
                  ...
                  <p className="text-gray-800 font-semibold leading-relaxed pr-4 mb-4 min-h-[50px]">
                    <span className="text-green-700">{menuData[activeDay].comboVeg}</span> + 3 Roti <span className="text-gray-400 mx-1">OR</span> Steamed Rice
                  </p>
                  <button 
                    onClick={() => handleAddToCart('Super Combo', `${menuData[activeDay].comboVeg} + 3 Roti OR Rice`, 50)}
                    className="flex items-center gap-1.5 text-sm font-bold bg-[#7CB342] text-white hover:bg-green-700 px-5 py-2.5 rounded-lg transition-colors shadow-md w-fit active:scale-95"
                  >
                    <Plus size={16} strokeWidth={2.5} /> Add to Order
                  </button>
                </div>
                ...
                <div className="bg-white/60 p-5 rounded-xl border border-gray-100 shadow-sm hover:shadow-md transition-shadow">
                  ...
                  <p className="text-gray-800 font-medium leading-relaxed pr-4 md:w-3/4 mb-4">
                    {menuData[activeDay].dinner}
                  </p>
                  <button 
                    onClick={() => handleAddToCart('Dinner', menuData[activeDay].dinner, 80)}
                    className="flex items-center gap-1.5 text-sm font-bold bg-[#7CB342]/10 text-[#7CB342] hover:bg-[#7CB342] hover:text-white px-4 py-2 rounded-lg transition-colors border border-[#7CB342]/30 w-fit"
                  >
                    <Plus size={16} strokeWidth={2.5} /> Add to Order
                  </button>
